(() => {
  ...
  // CISA's cybersecurity advisories feed. Closest reasonable free source for
  // "ongoing cyber attacks" — there's no single clean free API for that; this
  // is authoritative, actively maintained, and CVE-adjacent (ICS/vendor
  // advisories, many tied to actively-exploited vulnerabilities).
  // Verified reachable and parseable as of this writing:
  //   https://www.cisa.gov/cybersecurity-advisories/all.xml
  const FEEDS = ["https://www.cisa.gov/cybersecurity-advisories/all.xml"];

  const MAX_ITEMS = 10;

  // CISA's feed has no CORS header, so — same as this file's previous
  // NPR-based version — go through an RSS->JSON proxy.
  const proxyUrl = (rssUrl) =>
    "https://api.rss2json.com/v1/api.json?rss_url=" +
    encodeURIComponent(rssUrl);

  // Standalone widget: own tiny escaper (see JS/cves.js) rather than a
  // dependency on Vestiges.escapeHtml — this renders third-party text.
  ...
  async function run() {
    root.innerHTML = "";
    root.appendChild(
      renderItem({ dateText: "—", title: "loading…", summary: "", link: "#" })
    );
    ...
    if (!items.length) {
      root.appendChild(
        renderItem({ dateText: "—", title: "no updates", summary: "", link: "#" })
      );
      return;
    }
    ...
    for (const it of items) {
      ...
      root.appendChild(
        renderItem({ dateText: it.dateText, title: it.title, summary: clipped, link: it.link })
      );
    }
  }
  ...
  run().catch((e) => {
    ...
    root.innerHTML = "";
    root.appendChild(
      renderItem({ dateText: "—", title: "advisory feed unavailable", summary: "", link: "#" })
    );
  });
})();
