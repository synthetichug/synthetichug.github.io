(() => {
  ...
  // per IP without one; see https://nvd.nist.gov/developers/vulnerabilities).
  // One request per page load, well under any reasonable free-tier limit.
  const MAX_ITEMS = 5;
  const WINDOW_DAYS = 8; // recently published, not the full historical set
  ...
  function nvdUrl() {
    ...
    return (
      ...
      `&resultsPerPage=${MAX_ITEMS * 4}` // over-fetch a little, sort, then trim
    );
  }

  // Standalone widget (like the render/*.js modules, but outside that
  // pattern by design) — keep its own tiny escaper rather than depending on
  // Vestiges.escapeHtml, since this fetches third-party text into innerHTML.
