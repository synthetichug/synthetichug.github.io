...
(() => {
  const root = document.querySelector<HTMLElement>("[data-newsstream]");
  if (!root) return;

  const FEEDS = ["https://www.cisa.gov/cybersecurity-advisories/all.xml"];
  const MAX_ITEMS = 10;

  const proxyUrl = (rssUrl: string): string =>
  ...
  function renderItem({ dateText, title, summary, link }: Partial<FeedItem>): HTMLElement {
  ...
  async function fetchFeed(rss: string): Promise<any> {
    const res = await fetch(proxyUrl(rss), { cache: "no-store" });
    if (!res.ok) throw new Error(`feed fetch failed (${res.status})`);
    return res.json();
  }

  function normalizeItems(feedJson: any): FeedItem[] {
    ...
    return items
      ...
      .filter((it: FeedItem) => it.title && it.link);
  }
