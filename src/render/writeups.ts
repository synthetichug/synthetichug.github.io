...
import type { WriteupsData, Writeup } from "../types.js";

export function renderWriteups(data: WriteupsData | Writeup[] | null | undefined): void {
  const root = qs("[data-writeups]");
  if (!root) return;

  const list: Writeup[] = Array.isArray(data) ? data : data?.writeups ?? [];
  clear(root);
