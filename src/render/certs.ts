...
import type { CertsData, Cert } from "../types.js";

export function renderCerts(data: CertsData | Cert[] | null | undefined): void {
  const root = qs("[data-certs]");
  if (!root) return;

  const certs: Cert[] = Array.isArray(data) ? data : data?.certs ?? [];
  clear(root);
